...
- **Shadow-transition rule (normative, ADR): confirmed `conn → not_applicable` and `resources → not_applicable` flips NEVER write a changelog row — suppression at write time, same as the bootstrap rule.** They are deterministic shadows of rows the feed already carries: `conn` only reaches `not_applicable` when base/www lose their AAAA (which writes its own row in the same confirmation window), and `resources` only reaches `not_applicable` when `conn` leaves `supported`. The flip itself still commits (status/`*_since` move; the Transition is reported to telemetry) — only the feed row is suppressed. Transitions **out of** `not_applicable` are genuine news and keep their rows.
  ...
  _Erratum 2026-09-02: the rule is now keyed on the **cause**, not the target value, for `resources` (also amends ADR 0002 item 2). Its premise — "`resources` only reaches `not_applicable` when `conn` leaves `supported`" — contradicts 02 §6's own roll-up, which returns `not_applicable` whenever the effective link set is empty: a dependency pruned at 30 days (`SQLPruneDomainResources`) or swept to `no_record`, with `conn` still `supported`. That flip clears `resources_v4only`, sets `saint` and turns `ipv6_only` `supported` — public news — yet wrote no row, while the reverse flip kept its row, so the per-domain changelog, the feeds and the history replay were asymmetric. **`resources → not_applicable` is a shadow only when `conn`'s post-step-2 status is non-nil and not `supported`.** A `conn` that was never confirmed does not suppress. `conn → not_applicable` is unchanged: always a shadow. The verdict is made once, in `ComputeCommit`, and carried as `Transition.Shadow` so crawler metrics counts the same rows the changelog does instead of re-deriving the rule. `TestCommitResourcesNotApplicableIsNews` and `TestCommitResourcesShadowFollowsConn` pin both halves; 12-frontend.md §11's table gains a matching erratum, since its `resources | not_applicable` row is no longer defensive-only._
