...
3. **Files with uuid** — DB backstop first. A file whose uuid does not exist in `campaign` is resolved by `SELECT uuid FROM campaign WHERE source_file = $file`:
   - **No row** — the path owns no campaign, so the file is new: ADOPT it, inserting `campaign` with the file's own uuid (this is also how the production-data migration lands, since the existing YAML files already carry production uuids; see 08-migration-cutover.md). Rejecting instead would strand the campaign indefinitely: the `"remove the uuid field"` route only registers a campaign where step 6's write-back can push the generated uuid back into the repo, and `campaign.push` is false wherever the sync runs from a mounted checkout (the production crawler included).
   - **Row exists under a different uuid** — the uuid was edited in place: REJECT and report (`"unknown uuid on a file that already has a campaign — a uuid edited in place; restore the old value or remove the uuid field"`). With step 5 disabling the old uuid in the same run, both halves are loud and nothing is silently renamed.
...
5. **Deletion (uuid-set diff, not source_file diff):** after steps 3–4:
   ...
   Log each disabled campaign. Membership rows are kept (soft delete, history preserved); orphaned rank-NULL domains are handled by the lifecycle sweep (§3.6). A restored YAML *without* a uuid is a new campaign per step 4 (new uuid written back); the old disabled row stays soft-deleted. Consequence, stated for the implementer: a uuid edited in place = old campaign disabled by this step + new uuid rejected by step 3's backstop (the path already owns a row) — both loudly in the report; nothing is silently renamed.
...
7. **Report** to the ops webhook: created/updated/renamed/re-enabled/disabled campaigns, membership adds/removes, rejected files with reasons (schema, hostname, size, duplicate uuid, in-place uuid edit), rejected host entries + reasons, write-back status (pushed / nothing to push / failed).
