...
2. **Fetch list ID:** `GET https://tranco-list.eu/top-1m-id` → plain-text list ID (whitespace-trimmed, verified format: non-empty, ≤16 chars, alphanumeric — anything else is treated as a network/HTTP error). Then:
   ...
   _Erratum 2026-09-02 (review issue 36): the first bullet is too narrow. It matches only the **most recent** successful row, so an id the endpoint republishes after a newer one has landed — a Tranco rollback — passed the check and imported: the process-memory ETag belongs to the newer artifact so the conditional GET returns 200, the older list upserts and delists, every rank reverts to that day's values, and step 12's `ON CONFLICT (list_id) WHERE NOT aborted DO NOTHING` writes no provenance row. Because the old row's `imported_at` stays older, `TrancoLatestSuccessListID` still names the newer list, so the churn repeats on every 2h cycle. The bullet now reads: **equals `list_id` of ANY `tranco_import` row with `aborted = false` → no new list; attempt done.** `--force` still bypasses this, as it does the aborted-list bullet and the step-9 guard. Step 12 carries the backstop: if the provenance INSERT affects **0 rows** the transaction rolls back with outcome `no_new_list` rather than committing a rank rewrite whose provenance says nothing happened — which is what makes `--force` safe here too. `TestTrancoRepublishedOlderList` in `internal/ingest` pins both._
...
Files considered: every `*.yml` / `*.yaml` at the repository **root** only (no subdirectories).
...
5. **Deletion (uuid-set diff, not source_file diff):** after steps 3–4:
   ...
   _Erratum 2026-09-02 (review issue 29): "seen in the repo this run" counted only files that **parsed**, so a rejected file was indistinguishable from a deleted one and one bad edit soft-deleted the campaign it described. One rule now covers all three shapes of "we could not read it": **a campaign is disabled only when its file is genuinely gone from a checkout we could read.**_

   1. _**A rejected or unparseable file freezes its campaign.** Its uuid joins the seen set — recovered by `rawFileUUID` from the raw bytes where the `uuid:` line survives the failure, else `CampaignUUIDBySourceFile` on the basename — and the campaign is reported under **`DisableFrozen`**, mirroring `CuratedFrozen`. The run completes; only the destructive half is held back._
   2. _**A file whose hosts all fail validation is a rejection.** `CampaignRemoveMembersNotIn(campaign, [])` is never issued: a file that listed hosts and produced none is a bad edit, and removing every member over it unlists the whole campaign. A genuinely empty `domains:` list still clears its members — that is a real edit, and it produces no rejected hosts._
   3. _**An empty checkout aborts the run.** Nothing parsed while the database still has enabled campaigns is a broken clone or a wrong `campaign.repo_path`, not a repo where every campaign was deleted: the sync fails with `campaign checkout is empty; refusing to disable N campaigns`, reported to the ops webhook, and writes nothing. `v6ctl campaign sync --force` is the deliberate override. A file removed from a **healthy** checkout still disables its campaign — that path leaves other files parsed, which is the distinction the guard draws._

   _`TestCampaignRejectedFileFreezesDisable`, `TestCampaignAllHostsRejectedFreezesRemoval`, `TestCampaignEmptyCheckoutAborts` and `TestCampaignEmptyCheckoutOnEmptyDB` in `internal/campaign` pin the four cases._
...
7. **Report** to the ops webhook: created/updated/renamed/re-enabled/disabled campaigns, membership adds/removes, rejected files with reasons (schema, hostname, size, duplicate uuid), rejected host entries + reasons, write-back status (pushed / nothing to push / failed).
   ...
   _Erratum 2026-09-02 (review issue 42): reporting is not enough on the `v6ctl campaign sync` path. **"Completed with rejections" exits non-zero** — any rejected file, any rejected host entry, `CuratedFrozen`, or a `failed:` write-back. The database work has already committed at that point, so the exit code changes only visibility, and visibility is the gap: the verb runs from the webhook-triggered CI job and under systemd `OnFailure`, both of which read the exit status and nothing else, and a rejected campaign file that never imports looked exactly like a clean sync to both. `tranco import` already exits non-zero on its aborted outcome. The daily-tick path is unaffected — it has no exit code — and its report still travels by log line. `TestSyncOutcome` in `cmd/v6ctl` pins the six cases._
...
- `--base` given (CI mode): changed-file set = `git -C <repo> diff --name-status --no-renames <base>...HEAD` filtered to root-level `.yml`/`.yaml`; runs all §4.2 checks including the UUID diff rule (reading each file's `uuid:` at `<base>` via `git show`).
- `--base` omitted (local mode): validates **every** root-level YAML file, skipping the UUID diff rule (no git required); cross-file duplicates still reported informationally.
...
_Erratum 2026-09-02 (review issue 34): "the same `publicsuffix-go` library §3.4 uses" was true of every path **except Tranco**, which derived `tld` from `golang.org/x/net/publicsuffix` — a second vendored PSL snapshot, a month apart from the first. Two snapshots can disagree about a second-level rule, so one suffix could split into two `?tld=` facets by ingress: Tranco rows under `xx`, campaign rows under `foo.xx`, and `?tld=foo.xx` missing every Tranco-sourced row._

_The single derivation now lives in **`internal/domain`**: `PSLParse` (weppos `publicsuffix-go`, `IgnorePrivate`, no wildcard default rule) moved there from `internal/campaign`, and `domain.TLD` is implemented on top of it. Every path — Tranco import, campaign sync, curated subdomains, the live-check row insert, and `POST /check` validation — goes through it. `internal/campaign` no longer exports `PSLParse`, and `internal/ingest` no longer imports `x/net/publicsuffix`. (`checker/dns_ns_ipv6.go`'s ICANN walk-up still uses `x/net` — it asks a different question and is not a `tld` writer.)_

_`domain.TLD` keeps a **final-label fallback** where `PSLParse` refuses. `domain.tld` is NOT NULL, the Tranco import admits rows through its own validation, and a suffix the PSL snapshot has not caught up with should land in a facet named after its label rather than fail the import. `PSLParse` itself still refuses an unknown TLD, which is what campaign validation depends on (§4.2)._

_**Backfill:** existing rows still carry whichever answer their ingress gave at insert time. `v6ctl domain retld --dry-run` reports how many rows and which kinds would change; without the flag it rewrites only the disagreeing rows, and it is idempotent — safe to re-run after a PSL bump. **`export.SchemaVersion` is deliberately not bumped by this change**: it versions the manifest shape and the exported column set (07 §5.3), and neither moved. If a backfill run reports a non-zero count, the exported *values* changed, and the operator should bump it then — the verb says so on completion._
...
**Decision:** `hosting_provider` is a denormalized normalized-TEXT tag (05-schema.md — domain), **not** an FK — the CDN-suffix map and the hosting-ASN map are code constants, and the ASN fallback already has its own `asn` row, so a second reference table would duplicate `asn`. Both maps are Go constants co-located with the commit-path attribution helpers, single-sourced with §11.2's CDN-suffix list. _Erratum 2026-09-02:_ the two clauses could not both hold — the CDN suffix→tag map is now the single literal `checker.CDNSuffixTags`, beside §11.2's list, and `ingest.NormalizeHosting` reads its tags from there; the hosting-ASN map stays co-located in `ingest`. Only **normalized** tags are written (raw `asn.name` org strings are deliberately not used — that would defeat a clean league facet). Registrar attribution is **deliberately not derived** here (RDAP cost at 1M scale — deferred, §10.3).
