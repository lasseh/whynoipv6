...
- **Cancelled context:** if `ctx.Err() != nil` before the check runs, store `Result{Status: StatusError, Details: {"error": "scan cancelled"}}`.
  ...
  _Erratum 2026-09-03 (review issue 68): this branch now also logs `check starved: domain budget expired before it ran` (fields: `domain`, `check`, `err`) at warn. It is the only place the engine can observe a check losing its slot race against `domainTimeout`, and it was silent — the stored detail says "scan cancelled" but the §14 `scan_detail` payload keeps only the status, so nothing downstream could name the check that lost. **It should never fire.** Worst case every check runs to its declared §11 timeout: phase 1 is 6 checks under a limit of 6 (15/15/25/30/20/5s, fully parallel → 30s) and phase 2 is 9 under 6 (30/30/30/20/15/15/10/10/10s), whose worst ordering starts the three 30s checks last, at t=10 when the first short slot frees, finishing at 40s. 30 + 40 = **70s against the 90s budget**, so no check can starve as the constants stand — and a per-check timeout raised or a tenth phase-2 check added is exactly what would break that, which is what the log line is for. `TestStarvedCheckIsLogged` pins the branch._
...
_Erratum 2026-09-02: one deliberate deviation from the **verbatim** port (§2's table). The lifted code reported `resp.ContentLength` whenever the header was present and `len(body)` only on a chunked or gunzipped response, so the two families measured different quantities — an uncapped header against a 1 MiB-capped read — whenever one edge framed the response differently. A 2 MiB page served identity to v4 and chunked to v6 compared 2097152 against 1048576 and reported `partial` at a 50% diff. `content_length` is now always the bytes actually read, which is what this section's own "body read up to `maxBodySize` for length measurement" says. `TestResponseParityMeasuresTheBytesItRead` in `internal/checker` pins it._

_Erratum 2026-09-02 (review issue 63): step (2)'s "`ValidateIP` both first addresses" and step (3)'s single pinned address per family are replaced by a bounded walk. The check tried `v4IPs[0]`/`v6IPs[0]` only, so a site announcing several AAAAs with a decommissioned first edge earned a **definitive `unsupported`** on the dimension that feeds `broken_v6` and `ipv6_only` — while every browser reached it on the next address via Happy Eyeballs, and three counted scans on the same dead address confirmed it. Each family is now walked in order for up to `maxAddressAttempts = 3` addresses; the first that answers wins. A blocked address is **skipped** rather than failing the check, and does not consume an attempt (a rotation with one address in a reserved range still has the others). `error` is reserved for the case where nothing was probed at all — every candidate blocked — because that is our refusal, not the site's failure. The v4-fails → `not_applicable` and v6-fails → `unsupported` mapping is unchanged; it now means "every address we were allowed to try failed". `TestResponseParityWalksTheAddressRotation` pins all three branches._

_Erratum 2026-09-03: step (3)'s failure mapping gains a timeout branch, and the erratum above is corrected. **A timeout on either family is `error`, not a verdict.** v4 fails → `not_applicable` and v6 fails → `unsupported` now mean "failed for a reason the far end gave us" — refused, reset, bad certificate. A timeout is our clock, not the site's answer, which is how every other check in the package already rules (`dial.go`'s `isTimeout` branch, §11.7, §11.10). The walk above made it matter: up to `maxAddressAttempts` dials per family at the dialer's 10 s against this section's 20 s, so one dead IPv4 address plus one slow IPv6 address exhausts the budget and the IPv6 half never gets to run — and `not_applicable` is a value, not an absence, so the v4 branch defers too. **Correction to the 2026-09-02 erratum:** it called this "the dimension that feeds `broken_v6` and `ipv6_only`". It is not. `parity` is informational (this section's own first line; 02-observation-model.md §7.4); `broken_v6` and `ipv6_only` derive from `conn`, which `composeConn` builds from `https_ipv6`. The issue-63 walk is still right, but its stated blast radius was wrong. `TestResponseParityDefersOnATimeout` pins both branches._

_Erratum 2026-09-03: the **v6** branch defers on an unreachable network too (`checker.isUnreachable` — `ENETUNREACH`/`EHOSTUNREACH`). "Failed for a reason the far end gave us" is exactly what an unreachable network is not: no address was tried and the site said nothing, so `unsupported` reads our own routing as its answer. The **v4** branch still returns `not_applicable` on a generic failure and is left alone here — it defers v4 timeouts on the same logic, so the asymmetry is known and open._
...
- IPv6-pinned page fetch: bulk `LookupAAAA(host)` → first IP, `ValidateIP`, GET `https://<host>/` with transport pinned to `[ip]:443` over `tcp6`, TLS config as §11.7, UA per §3, up to 3 redirects followed then last response (all redirect requests re-dial the same pinned address — lifted behavior, kept), body read capped at `resourceMaxBodySize = 2 MiB`.
  ...
  _Erratum 2026-09-02: "all redirect requests re-dial the same pinned address" cannot hold across a host change, and the code that enforced it made **apex→www unfollowable** — the dominant apex configuration. Discovery then parsed the 3xx boilerplate, returned `supported` with zero hosts, and the roll-up folded that empty set into a confirmed `resources = not_applicable`, which unlocks `saint` and `ipv6_only = supported` over a page never read (review issue 01, finding 09-0). The shipped rule: a redirect whose target is **the apex itself or a subdomain of it**, over the same scheme and port, is followed as a **new pinned fetch** — the target's own `LookupAAAA` through the bulk resolver, its own `ValidateIP`, its own pin and SNI. Redirects within a host are still followed by the pinned client. Redirects **off the site** stay unfollowed: those genuinely would fetch the wrong vhost, which is what the restriction was added for. The hop budget is shared: `discoveryMaxRedirects` total across same-host and in-scope cross-host hops._

  _Erratum 2026-09-02 (review issue 30): two admission limits on the same bullet. (a) **Scheme allowlist.** The lifted code skipped `data:` and `javascript:` by name and let every other scheme resolve to a host — `mailto:`, `tel:`, `blob:`, `ws:`, `chrome-extension:` and whatever comes next. Only **`http` and `https`** are counted now; an allowlist needs no extending for the next scheme. (b) **Per-registrable-domain cap of 5.** A page that shards its assets (`img1..imgN.cdn.example`) or a tracker minting a per-visit subdomain used to fill the whole `resourceMaxHosts = 50` budget with one operator's names — crowding out genuinely distinct dependencies and minting a `resource_host` row per sighting. `maxHostsPerSite = 5` distinct hosts per eTLD+1 per page, keyed with `publicsuffix.EffectiveTLDPlusOne` so a multi-label suffix caps on `example.co.uk`, not `co.uk`. The `resourceMaxHosts` ceiling still applies on top. A per-day admission cap on registry rows is **not** part of this — registry growth is issue 04's GC. `TestResourceDiscoverySchemeFilter` and `TestResourceDiscoveryPerSiteCap` pin both._

_Every hop is over IPv6 or not at all._ Phase 2 gates discovery on the entity host having AAAA, and a redirect target is held to the same rule: no AAAA, an unresolvable name, or an address the SSRF blocklist rejects means the hop is not taken and the **last response stands**. Discovery never falls back to IPv4 to find a page — a v4-only `www` is the `www` dimension's story, and fetching it over v4 here would let a v4-only page hide behind an apex that has AAAA. `TestResourceDiscoveryFollowsApexToWWW`, `TestResourceDiscoveryKeepsCrossSiteRedirects` and `TestResourceDiscoveryNeedsAAAAOnTheHop` in `internal/checker` pin the three rules.
...
_Erratum 2026-09-02 (review issue 63): the final sentence's "otherwise `unsupported` with the last error message" no longer covers a **timeout**, which now yields `error`. Connection-refused stays `unsupported` — the host answered, and what it said was no — but a dial or read that timed out is not evidence about the MX. This section calls the check informational; `mx` is not. It sits in the 03 §10 hero bar, so a false `unsupported` costs a domain its hero status and raises `mail_missing`. And outbound port 25 is filtered by most cloud providers: an egress change would time out identically for every domain in the frontier and, after two counted scans, mark the internet's mail IPv6-unsupported. `error` defers the dimension instead, leaving the confirmed value untouched, and carries no lifecycle cost — `error_streak` is driven by base and www alone (03 §5 step 5). `TestSMTPIPv6/timed-out MX is error, not unsupported` pins it._

_Erratum 2026-09-03: the deferral extends to an **unreachable network**. `ENETUNREACH`/`EHOSTUNREACH` (`checker.isUnreachable`) is our own kernel declining to route the dial — the MX never saw it, so it is not evidence, for the same reason a timeout is not. It is the sharper case: a host that loses its IPv6 route marks every MX in the frontier `unsupported` at dial speed rather than slowly, and the crawler's IPv6 self-preflight only gates a total loss, not a missing route to one prefix. Connection-refused is untouched. This is also why `TestSMTPIPv6/timed-out MX is error, not unsupported` passes on a runner with no global IPv6: the RFC 6666 discard address it dials returns `ENETUNREACH` there and times out here, and both arms now rule the same way._
