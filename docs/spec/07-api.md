...
_Erratum 2026-09-03 (review issue 60): step 2's per-IP count skips jobs the reaper failed before any consumer claimed them — the predicate gains `AND NOT (status = 'failed' AND claimed_at IS NULL)`. Such a job consumed no crawler work by construction, and it reached that state precisely because the system was failing the caller: the crawler was down, or the queue was deeper than `live_check.fail_after`. Billing an hour's lockout for an outage the caller did not cause, with `retry_after` telling them to wait it out, is not what the quota is for. `claimed_at` is written only by the §5.1.5 claim and never reset, so the predicate names exactly that population — `CheckJobFail` is called by a consumer, which has claimed by definition. **Everything a consumer claimed still counts, however it ended**, so a caller cannot submit hosts engineered to burn a 90s `domainTimeout` and fail, then use the failure to buy unlimited requests. `retry_after`'s `min(created_at)` is taken over the same filtered rows, so the two stay consistent. **The global count is deliberately unchanged** and still counts every row including the unclaimed ones: the per-IP limit is fairness to a caller, while the global limit is system backpressure, and a queue full of unclaimed jobs is exactly when backpressure is wanted. `TestRateLimitSkipsUnclaimedReapedJobs` pins both halves._
...
_Erratum 2026-09-03: the fixed-window surfaces — the feeds and the scoped recent-window lists — seed from **both ends of the window plus its row count**, not `max(changelog.ts)` alone. `changelog.ts` is the worker-fixed scan start, so a slow scan lands a row with a ts older than a faster, later-started scan's, and `max(ts)` never moves; the window gains an item and a client 304s over it, self-healing only when a strictly newer transition lands — days, for a sparse scope. The count covers that while the window is under `feed.recent_window`. At the cap — where the global feed sits permanently — an insertion is also an eviction, so the count cannot move either and the oldest row is the only thing that does. All three components are cheap: the handler already holds the rows before it caches. The **paginated** changelog lists are unaffected and still use the table-wide `max(ts)`; they cache before they know their window, so over-invalidating is the only safe direction there. `TestChangelogWindowValidator` and `TestChangelogWindowValidatorAtTheCap` pin the sparse and saturated cases._
