...
_Erratum 2026-09-02: §2.2's "migrations never call `refresh_continuous_aggregate()`" is correct but has an operational consequence it does not state. `changelog_daily` is created `WITH NO DATA` (a cagg cannot be materialized inside a transaction) and its refresh policy only covers `[now − 90 days, now − 1 hour]`; `materialized_only = false` unions raw rows **newer** than the materialization watermark, not older. On a deployment that has been running since before its first `changelog` row this is invisible. Restore a backup taken after the aggregate existed, or migrate against a copied `changelog` table, and buckets older than 90 days are neither materialized nor unioned — `/stats/changes` returns an empty range there and the 730-day floor presents it as a legitimate answer. **`CALL refresh_continuous_aggregate('changelog_daily', NULL, NULL)` once after any restore or re-migration**, before the API is reachable: `docs/runbooks/timescale-jobs.md` — "After a restore, or a migration against copied data"._

_Erratum 2026-09-02: §1.1's "all DDL is single-sourced here" did not hold for indexes. `create_hypertable` defaults `create_default_indexes` to **true**, so migration 000002 also created `scan_ts_idx`, `scan_detail_ts_idx`, `crawler_metrics_ts_idx`, `unbound_stats_ts_idx` and `stats_asn_daily_day_idx`, none of them declared anywhere. Migration 000011 drops the first two (no reader uses them; both scan readers go through the `(domain_id, ts)` primary key) and keeps the other three on purpose — the last is load-bearing for `StatsTopNetworks`. `TestMigrations` now pins the per-hypertable index set._
...
|---|---|---|
| `db/query/domain.sql` | frontier claim, commit UPDATE, entity lookups/ensure, subdomains, disable/enable — **not** the `/domains` list family or `?q=` search, which are builder-built (below) | 03, 04, 06, 07 |
...
| `db/query/asn.sql` | asn ensure/lookup, counter recompute, sentinel lookup — **not** the leaderboard keyset walks, which are builder-built (below) | 03, 06, 07 |
...
**Decision — the `/domains` list family is builder-built, not sqlc (the one carve-out).** The leaderboard/tier/scoped list endpoints and `?q=` search (07-api.md — §3.3, §4.4; roughly ten routes) are the single slice static sqlc cannot serve: their filter grammar implies **150–300 distinct SQL texts** if enumerated, and the collapse into one optional-param query (`$1 IS NULL OR classification = $1`) **defeats the partial indexes** — the planner's predicate-implication check needs the literal (`classification = 'hero'`) spelled in the text (§2 rule 7), a bind parameter cannot prove it, and pgx's prepared statements flip to a generic plan after ~5 executions, permanently locking in the full scan. These queries are therefore **built at request time with `github.com/Masterminds/squirrel`** in the hand-written adapter (`internal/postgres/domainlist.go`), emitting the scope predicate, the single residual (07 §3.3), the seek tuple, and `AND rank IS NOT NULL AND NOT disabled` as **verbatim literals** per request, scanned with `pgx.CollectRows`/`RowToStructByName`. (squirrel is API-stable in maintenance mode; the actively maintained `n-r-w/squirrel` fork is the drop-in successor if it ever blocks.) **Amendment (post-`prev_cursor`):** the builder also serves the two other keyset-walked lists — the paginating changelog feed (`internal/postgres/changeloglist.go`) and the ASN leaderboard (`internal/postgres/asnlist.go`) — on a second justification: bidirectional cursors would otherwise force a flipped-comparison/flipped-ORDER `*Prev` copy of every walk text (and a sort×direction matrix on `/asns`), and the direction flip is single-sourced in one builder per view instead. The carve-out remains bounded to these **keyset list walks** — every other query in the tables above stays sqlc; nothing else may adopt the builder without a spec change.

### 10.3 Package layout and the batch-commit pattern

```
internal/postgres/     hand-written adapters: the keyset list builders (§10.2) + Generation
internal/postgres/db/  sqlc output (generated; never hand-edited; regenerated by `sqlc generate`)
```

- Layering (amended in implementation): `domain` ← `postgres` ← {`crawler`, `api`, `cmd`}. The originally planned `repository` port-interface layer and the `service` pass-through were **deleted** — both failed the deletion test: the sqlc-generated `db.Queries` plus the `internal/postgres` builders ARE the data-access interface, and consumers hold `*db.Queries`/the pool directly. A ports layer returns only if a second adapter (e.g. an in-memory fake) actually materializes.
