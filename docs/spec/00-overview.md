...
whynoipv6/                     # monorepo root ("whynoipv6-new" is the seed)
  ...
  backend/                     # the Go module — all `make` targets run from here
    go.mod                     # module github.com/lasseh/whynoipv6; codeberg.org/miekg/dns pinned exact
    go.sum
  ...
  docs/
    backend-design.md          # authoritative design (Round 2.0) — input to this spec set
...
- **Gold.** A boolean refinement, not a class: `gold = (classification == hero AND resources ∈ {supported, not_applicable})`. It marks heroes that also serve their sub-resources over IPv6. Evaluates false everywhere while the resources feature is disabled. [03]
