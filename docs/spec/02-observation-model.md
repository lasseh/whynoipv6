...
**Known edge (accepted):** a provider that *filters* a name rather than failing (e.g. Quad9 answering NXDOMAIN for a blocklisted domain) is normally outvoted 2-1 (`Disagreed=true`). If the same lookup also loses a second provider to a non-answer, the surviving 1-1 split is no-quorum → `inconsistent`, and the domain cannot confirm while both conditions persist. This is accepted: the affected set is filter-listed names we are comfortable never confirming, `inconsistent` rides the fast recheck lane (04-lifecycle-scheduling.md — §5.2), and the persistent 2-1 disagreement remains visible in the per-provider quorum records below.
...
_Erratum 2026-09-02: the trigger takes **two** explicit answers, not one. "Every provider that answered returned SERVFAIL or REFUSED" was implemented as "at least one did, and none returned anything else", so one provider's validation hiccup alongside two timeouts ran the rescue and could credit a definitive `supported` from a single un-quorumed local-Unbound answer. The rule is now: **at least two** providers returned an explicit SERVFAIL/REFUSED **and** no provider returned any other rcode. A timeout still does not disqualify — it just does not count toward the two, so one silent provider is fine and two are not. `consensus.cdRescueQuorum` is the constant. This stays deliberately below `AAAADetail.ExplicitlyUnresolvable`'s three-of-three, which serves 03-state-machine.md §4 branch (b) — a different rule, on the dead-detection side. 10-testing.md §3.2b's `cd_notrun_timeout` row is correct as written and now passes; `TestConditionalCDLookup` in `internal/consensus` carries it plus `cd_runs_on_two_servfail_plus_timeout` and `cd_notrun_mixed_rcode`._
...
- **Open (suppress):** when `total ≥ min_samples` (500) AND `nondefinitive/total > nondefinitive_rate` (0.05) over the trailing window → set suppressed, alert the ops webhook, log `level=warn` with a `msg` prefixed `fastlane breaker open`. Both lines carry the same diagnostic set: `nondefinitive`/`samples` counts, the `rate` rounded to three decimals, the `threshold` and `window` in force, and `provider_nonanswers` — each provider's non-answer count over its own §2.10 window, which separates "one resolver is sick" from "all three answered and disagreed" without a second query.
...
- **Wrong-but-answering providers are outvoted, not dropped — but they are counted.** The breaker triggers only on non-answers; a provider persistently returning a *wrong valid symbol* (anycast/ECS drift, filtering) never trips it. So the worker tallies per-provider disagreement counts (a valid answer whose symbol differs from the quorum symbol) into the checkpoint metrics (`crawler_metrics.dim_counters`, keys `consensus_disagree_{cloudflare|google|quad9}`; 05-schema.md — crawler_metrics), making a drifting provider visible on the ops dashboard (09-ops.md — Grafana). No automatic action is taken — 2-of-3 already outvotes it; the tally exists so a human notices a systematic pattern.
