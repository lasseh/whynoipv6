...
3. **DomainDetail** — breadcrumb, host + Provider/Rank header (rank badge `bg-fuchsia-900`), Domain Status card: `RatingStars` (4 stars, §7.3) + the §7.1 accordion (four dimension rows + the IPv6 Only fold row) each expanding a `Tracker` (§7.3) + "Last checked", then `SubdomainTable` from `/domains/{host}/subdomains` (a fourth non-fatal side surface, fetched only when the detail's `subdomain_count > 0` and rendered once its rows arrive, so a childless domain issues no request and shows no section), then `ChangelogTable` from `/domains/{host}/changelog`. Unknown host → §6.3 not-found redirect.
   ...
   **When `kind = 'subdomain'`** the page drops the `www` row *and* its star, rating on three dimensions instead of four: the engine never resolves `www.<subdomain>` and stores a fixed `not_applicable` (01 §runner), so rendering it would invent a hostname nobody looked up. The base row's copy says "This hostname" rather than "The apex domain". The parent is surfaced twice — as a breadcrumb crumb and as a "Subdomain of `<parent>`" line under the provider — because the hostname alone does not tell a reader which domain owns it. `CampaignDomain` carries the same header line, linking out of the campaign scope.
