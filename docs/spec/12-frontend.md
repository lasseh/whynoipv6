...
_Erratum 2026-09-02: the `resources | not_applicable` row is **no longer defensive-only** (03 §11 erratum, review issue 02). A domain that drops its last v4-only dependency confirms `unsupported → not_applicable` with `conn` still `supported`, which now writes a changelog row and renders in the feeds. The `conn | not_applicable` row stays defensive-only. **Open copy question, deliberately not decided here:** the sentence reads as a downgrade ("no longer has its page resources checked") for what is in fact a saint-restoring gain, and the emerald/zinc colour follows it. `TestFeedItemTitleGoldens` pins the shipped sentence as `resources dependency dropped`; changing it means changing this table, the Go golden and `frontend/src/utils/__tests__/changelog.test.ts` together._
...
3. **DomainDetail** — breadcrumb, host + Provider/Rank header (rank badge `bg-fuchsia-900`), Domain Status card: `RatingStars` (4 stars, §7.3) + the §7.1 accordion (four dimension rows + the IPv6 Only fold row) each expanding a `Tracker` (§7.3) + "Last checked", then `SubdomainTable` from `/domains/{host}/subdomains` (a fourth non-fatal side surface, fetched only when the detail's `subdomain_count > 0` and rendered once its rows arrive, so a childless domain issues no request and shows no section), then `ChangelogTable` from `/domains/{host}/changelog`. Unknown host → §6.3 not-found redirect.

   **When `kind = 'subdomain'`** the page drops the `www` row *and* its star: the engine never resolves `www.<subdomain>` and stores a fixed `not_applicable` (01 §runner), so rendering it would invent a hostname nobody looked up. The `mx` row goes too, but **only when its value is `not_applicable`** — MX is not www. The check genuinely runs for subdomains, and one publishing its own MX gets a real verdict (dev: 245 of ~1,455 subdomains, `graph.facebook.com` and `m.youtube.com` among them); only the no-MX case is a structural skip, since the engine grants no implicit-MX fallback there (01-engine.md §11.4). An apex keeps its `mx` row at `not_applicable`, where "no mail configured" is a real answer a Hero can hold. So a subdomain rates on two to three dimensions, and the star count always equals the row count. The base row's copy says "This hostname" rather than "The apex domain". The parent is surfaced twice — as a breadcrumb crumb and as a "Subdomain of `<parent>`" line under the provider — because the hostname alone does not tell a reader which domain owns it. `CampaignDomain` carries the same header line, linking out of the campaign scope.

   `SubdomainTable` applies the same rule column-wise: the Mail (MX) column renders only when at least one listed child has a verdict other than `not_applicable`, so it disappears for the common all-dashes case without hiding a child that does run mail.
4. **CampaignDomain** — DomainDetail variant: campaign breadcrumb, changelog scope `/campaigns/{uuid}/domains/{host}/changelog`, same `SubdomainTable` (child links always leave the campaign scope — a child need not be a member).
5. **Search** — input + single "Domains" result table from `/domains?q=`, rank-ordered (rank-NULL matches last), cursor-paged via §9.1 (the old page showed one unpaged list; results past 50 are now reachable through the same Previous/Next control — §7.3 covers the campaign-domain fold-in).
...
| OPEN-F7 | Search couldn't see campaign-only (rank-NULL) hosts (review blocker B2 — the `/domains` default scope excludes them, contradicting §7.3's fold-in premise). | **Resolved 2026-07-11: API amended** — 07 §3.3 `?q=` spans rank-NULL, non-disabled rows. **Narrowed 2026-08-22:** rank ordering sorts them behind every ranked match, so the guarantee is "findable", not "on page 1" |
