...
3. **DomainDetail** — breadcrumb, host + Provider/Rank header (rank badge `bg-fuchsia-900`), Domain Status card: `RatingStars` (4 stars, §7.3) + the §7.1 accordion (four dimension rows + the IPv6 Only fold row) each expanding a `Tracker` (§7.3) + "Last checked", then `SubdomainTable` (only when `subdomain_count > 0`), then `ChangelogTable` from `/domains/{host}/changelog`. Unknown host → §6.3 not-found redirect.
