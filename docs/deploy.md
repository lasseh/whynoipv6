...
| `logs [-u unit…] [-f] [-n N] [--since …] [--level …]` | pretty-print a unit's JSON logs from journald (or stdin with `-`); `-u` is repeatable and short names expand to globs, so `crawler` covers both templated processes; needs no DB (09-ops §13) |
