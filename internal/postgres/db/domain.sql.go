...
const GetDomainLog = `-- name: GetDomainLog :many
SELECT id,
       time,
       data
FROM domain_log
...
type GetDomainLogRow struct {
	ID   int64
	Time time.Time
	Data pgtype.JSONB
}

func (q *Queries) GetDomainLog(ctx context.Context, domainID int64) ([]GetDomainLogRow, error) {
	rows, err := q.db.Query(ctx, GetDomainLog, domainID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetDomainLogRow{}
	for rows.Next() {
		var i GetDomainLogRow
...
const StoreDomainLog = `-- name: StoreDomainLog :exec
INSERT INTO domain_log(domain_id, data)
VALUES ($1, $2)
RETURNING id, domain_id, time, data
`

type StoreDomainLogParams struct {
	DomainID int64
	Data     pgtype.JSONB
}

func (q *Queries) StoreDomainLog(ctx context.Context, arg StoreDomainLogParams) error {
	_, err := q.db.Exec(ctx, StoreDomainLog, arg.DomainID, arg.Data)
	return err
}
