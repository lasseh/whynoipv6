...
const GetCampaignDomainLog = `-- name: GetCampaignDomainLog :many
SELECT id,
       time,
       data
...
type GetCampaignDomainLogRow struct {
	ID   int64
	Time time.Time
	Data pgtype.JSONB
}

func (q *Queries) GetCampaignDomainLog(ctx context.Context, domainID int64) ([]GetCampaignDomainLogRow, error) {
	rows, err := q.db.Query(ctx, GetCampaignDomainLog, domainID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetCampaignDomainLogRow{}
	for rows.Next() {
		var i GetCampaignDomainLogRow
		if err := rows.Scan(&i.ID, &i.Time, &i.Data); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const StoreCampaignDomainLog = `-- name: StoreCampaignDomainLog :exec
INSERT INTO campaign_domain_log(domain_id, data)
VALUES ($1, $2)
RETURNING id, domain_id, time, data
`

type StoreCampaignDomainLogParams struct {
	DomainID int64
	Data     pgtype.JSONB
}

func (q *Queries) StoreCampaignDomainLog(ctx context.Context, arg StoreCampaignDomainLogParams) error {
	_, err := q.db.Exec(ctx, StoreCampaignDomainLog, arg.DomainID, arg.Data)
	return err
}
