...
// TestAddHostSchemes pins the scheme allowlist in addHost. Only http and
...
func TestAddHostSchemes(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			var hosts []string
			addHost(tc.raw, base, "example.com", map[string]struct{}{}, &hosts)

			if tc.want == "" {
				if len(hosts) != 0 {
					t.Fatalf("addHost(%q) = %v, want no hosts", tc.raw, hosts)
				}
				return
			}
			if len(hosts) != 1 || hosts[0] != tc.want {
				t.Fatalf("addHost(%q) = %v, want [%s]", tc.raw, hosts, tc.want)
			}
		})
	}
}
